...
module.exports = async (messages, sock, store) => {
  ...
  if (messages.key) {
    m.key = messages.key;
    m.cht = m.chat = m.key.remoteJid.startsWith("status")
    ...
    if (m.msg?.contextInfo?.quotedMessage) {
      ...
      if (m.quoted.message) {
        ...
        m.quoted.isBot =
          ...
          m?.quoted?.id?.startsWith("F3FD") ||
          m?.quoted?.id?.startsWith("SSA") ||
          m?.quoted?.id?.startsWith("BAE5") ||
          m?.quoted?.id?.startsWith("B1EY") ||
          m?.quoted?.id?.startsWith("HSK") ||
          m?.quoted?.id?.indexOf("-") > 1;
  ...
  m.reply = async (text, options = {}) => {
    ...
    m.fkontak = {
      ...
      m.froll = async (number = config.owner[0], name = Func.Styles(config.name), thumbnail = sock.resize(fs.readFileSync('./image/Deku-Midoriya.jpg'), 300, 300)) => {
          const hmm = {
              key: {
                remoteJid: number + '-1625305606@g.us',
                participant: '0@s.whatsapp.net'
              },
              message: {
                 orderMessage: {
